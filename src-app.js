...
class Component extends DCLogic {
  ...
  // From the map's side panel: open the Timeline tab with this event expanded for the full details.
  openInTimeline(i){ this.setState({ panel:'timeline', expanded:i, selIcon:null }); }

  // Advance the journey to the next stop from within the Timeline (no map animation needed since the
  // overlay covers it): bump progress, move the ship + selection to the new world, expand it, persist.
  advanceInTimeline(){
    const { progress } = this.state;
    if (progress >= this.beats.length - 1) return;
    const next = progress + 1, toW = this.beats[next].w, to = this.meta[toW];
    this.setState({ progress: next, selWorld: toW, selIcon: null, expanded: next, shipVisible: true, ship: { x:to.x, y:to.y } }, this.persist);
  }
  ...
  renderVals(){
    ...
    if (selWorld){
      ...
      selBeats = wb.map(({ b, i }) => {
        ...
        return {
          ...
          // jump from the map's side panel into this event's expanded Timeline entry
          canViewTimeline: reached,
          viewTimelineStyle: { marginTop:'1.2cqh', display:'inline-block', fontFamily:"'Space Mono',monospace", fontSize:'1.2cqh', letterSpacing:'.14em', color:'#c89bff', cursor:'pointer', borderBottom:'1px solid rgba(200,155,255,.4)', paddingBottom:'.2cqh' },
          onViewTimeline: ()=> this.openInTimeline(i),
        };
      });
    }
    ...
    // "next stop" control inside the timeline — advance the journey without returning to the map
    const timelineCanContinue = started && progress < total - 1;
    return {
      ...
      timelineCanContinue,
      timelineNextTitle: timelineCanContinue ? this.beats[progress + 1].title.toUpperCase() : '',
      onTimelineContinue: ()=> this.advanceInTimeline(),
      timelineContinueStyle: { flex:'none', marginTop:'1.6cqh', alignSelf:'center', fontFamily:"'Oxanium',sans-serif", fontWeight:700, fontSize:'1.55cqh', letterSpacing:'.16em', color:'#0a0e14', background:'linear-gradient(90deg,#d9c2ff,#c89bff)', border:'none', padding:'1.1cqh 3cqh', cursor:'pointer', boxShadow:'0 0 1.8cqh rgba(200,155,255,.4)' },
      worlds, bodyLabels,
